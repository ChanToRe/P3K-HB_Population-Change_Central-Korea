...
data <- subset(data,subset = Type != "신라(방형)" | Type != "신라(呂·凸자형)")
...
# Plot
tiff(file=here("./Graph/Fig3-3(2)_dexp_HPD(신라제외)).tiff"), units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE)
title(main="신라제외(dExp-model):r1", cex=1.2)
...
title(main="신라제외(dExp-model):r2", cex=1.2)
...
title(main="신라제외(dExp-model):c", cex=1.2)
...
tiff(file=here("./Graph/Fig3-4(2)_dexp_model(신라제외).tiff"), units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file=here("./Graph/NotUsed/dexp_fit(신라제외).tiff"), units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
postHPDplot(postPredCor(pp.check.double_exp.cal), xlab="신라제외 PCC",ylab='Density',xlim=c(0,1))
dev.off()
