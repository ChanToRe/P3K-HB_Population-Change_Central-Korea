...
# Plot
tiff(file=here("./Graph/Fig3-3(1)_dexp_HPD(중부지역)).tiff"), units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
...
tiff(file=here("./Graph/Fig3-4(1)_dexp_model(중부지역).tiff"), units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file=here("./Graph/NotUsed/dexp_fit(중부지역).tiff"), units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
