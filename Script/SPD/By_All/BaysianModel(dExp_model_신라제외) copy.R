...
# Plot
tiff(file=here("./Graph/dexp_HPD(신라제외)).tiff"), units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
...
tiff(file=here("./Graph/dexp_model(신라제외).tiff"), units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file=here("./Graph/dexp_fit(신라제외).tiff"), units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
