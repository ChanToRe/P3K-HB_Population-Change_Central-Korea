...
data <- subset(data,subset = Type == "백제(방형)" | Type == "신라(방형)")
...
# Plot
tiff(file="C:/github/P3K-HB_Demography/Graph/dexp_HPD(마한문화권).tiff", units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE)
title(main="마한문화권(dExp-model):r1", cex=1.2)
...
title(main="마한문화권(dExp-model):r2", cex=1.2)
...
title(main="마한문화권(dExp-model):c", cex=1.2)
...
tiff(file="C:/github/P3K-HB_Demography/Graph/dexp_model(마한문화권).tiff", units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file="C:/github/P3K-HB_Demography/Graph/dexp_fit(마한문화권).tiff", units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
postHPDplot(postPredCor(pp.check.double_exp.cal), xlab="마한문화권 PCC",ylab='Density',xlim=c(0,1))
dev.off()
