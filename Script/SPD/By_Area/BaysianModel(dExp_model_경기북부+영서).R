...
data <- subset(data,subset = Area == "경기북부" | Area == "영서")
...
# Plot
tiff(file=here("./Graph/Fig3-7(1)_dexp_HPD(경기북부+영서).tiff"), units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE)
title(main="경기북부+영서(dExp-model):r1", cex=1.2)
...
title(main="경기북부+영서(dExp-model):r2", cex=1.2)
...
title(main="경기북부+영서(dExp-model):c", cex=1.2)
...
tiff(file=here("./Graph/Fig3-8(1)_dexp_model(경기북부+영서).tiff"), units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file=here("./Graph/NotUsed/dexp_fit(경기북부+영서).tiff"), units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
postHPDplot(postPredCor(pp.check.double_exp.cal), xlab="경기북부+영서 PCC",ylab='Density',xlim=c(0,1))
dev.off()
