...
data <- subset(data,subset = Area == "경기남부" | Area == "호서북부")
...
# Plot
tiff(file=here("./Graph/Fig3-7(2)_dexp_HPD(경기남부+호서북부).tiff"), units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE)
title(main="경기남부+호서북부(dExp-model):r1", cex=1.2)
...
title(main="경기남부+호서북부(dExp-model):r2", cex=1.2)
...
title(main="경기남부+호서북부(dExp-model):c", cex=1.2)
...
tiff(file=here("./Graph/Fig3-8(2)_dexp_model(경기남부+호서북부).tiff"), units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file=here("./Graph/NotUsed/dexp_fit(경기남부+호서북부).tiff"), units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
postHPDplot(postPredCor(pp.check.double_exp.cal), xlab="경기남부+호서북부 PCC",ylab='Density',xlim=c(0,1))
dev.off()
