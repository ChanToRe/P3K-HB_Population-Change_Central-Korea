...
title(main="한강 이남(dExp-model):r2", cex=1.2)
mtext(TeX('$r_2$'),side = 1,line=1.5,cex = 1, padj=1)
postHPDplot(abs(round(BPtoBCAD(mcmc.double_exp.samples$samples$chain1[, 'chp']))), xlab='', ylab='', show.hpd.val=FALSE, xlim=c(250, 450))
title(main="한강 이남(dExp-model):c", cex=1.2)
