...
# Increase Period Model Test(Exponential / 1000 simulations)
set.seed(12345)
expMod <- modelTest(x=MC_caldates, bins=MC_bins, errors=MC_data$Error, timeRange=c(2200, 1500), model="exponential", nsim=1000, runm=50)
plot(expMod, calendar="BCAD", lwd=2)
plot(spd.raw, add=TRUE, calendar="BCAD", type="simple", col="Navy", lwd=1, lty=2)
title("Model Test(Exponential)")
...
plot(expMod, calendar="BCAD", type='roc') +
  title("Local Growth Rates Test(Exponential)")
