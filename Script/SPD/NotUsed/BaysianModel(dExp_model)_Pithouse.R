...
obs.data <- data.frame(CRA=CRA,
                       Error=Errors,
                       MedCalDate=medDates,
                       SiteID=SiteID)

# Define Model
modelPlot(model=dDoubleExponentialGrowth,
          a=2200,
          b=1200,
          params=list(r1=rnorm(1000, sd=0.0004),
                      r2=rnorm(1000, sd=0.0004),
                      mu=round(runif(1000,1200,2200)
                      )))
...
data(intcal20)
constants <- list(N=length(obs.caldates),
                  calBP=intcal20$CalBP,
                  C14BP=intcal20$C14Age,
                  C14err=intcal20$C14Age.sigma)
data <- list(X=obs.data$CRA,
             sigma=obs.data$Error)

# Define Initialisation Function
initsFunction.double_exp <- function() list(r1=rnorm(1, sd=0.0004),
                                            r2=rexp(1, 1/0.0004),
                                            chp=round(rtruncnorm(1, mean=1700, sd=100, a=1200, b=2200)),
                                            theta=as.numeric(obs.data$MedCalDate))

# Run MCMC ####
mcmc.double_exp.samples<- nimbleMCMC(code=double_exp,
                                     constants=constants,
                                     data=data,
                                     niter=10000,
                                     nchains=3,
                                     ...
                                     inits=initsFunction.double_exp,
                                     setSeed=c(1, 2, 3))
...
# Plot
tiff(file="/Users/jch/Documents/github/P3K-HB_Demography/Graph/HPD(Pithouse).tiff", units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE) +
  title(main="dExp-model(Pithouse):r1", cex=1.2)
...
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r2'], xlab='', ylab='', show.hpd.val=FALSE)
  title(main="dExp-model(Pithouse):r2", cex=1.2)
...
postHPDplot(abs(round(BPtoBCAD(mcmc.double_exp.samples$samples$chain1[, 'chp']))), xlab='', ylab='', show.hpd.val=FALSE, xlim=c(150, 350))
  title(main="dExp-model(Pithouse):c", cex=1.2)
...
tiff(file="/Users/jch/Documents/github/P3K-HB_Demography/Graph/Baysian Model(dExp_model)_Pithouse.tiff", units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
postHPDplot(pp.check.double_exp.cal, xlab="Pearson's Correlation coefficient",ylab='Density',main='m2 goodness-of-fit',xlim=c(0,1))
dev.off()
