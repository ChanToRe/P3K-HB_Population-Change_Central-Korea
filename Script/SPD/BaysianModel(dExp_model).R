...
# Plot
tiff(file="/Users/jch/Desktop/dexp_HPD.tiff", units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE)
title(main="DoubleExp-model:r1", cex=1.2)
...
title(main="DoubleExp-model:r2", cex=1.2)
...
title(main="DoubleExp-model:c", cex=1.2)
...
tiff(file="/Users/jch/Desktop/dexp_model(7-5).tiff", units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file="/Users/jch/Desktop/dexp_model.tiff", units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
plot(pp.check.double_exp.cal, interval = 0.95,calendar='BCAD')
legend('topleft',legend=c('95% Prediction Interval','Positive Deviation','Negative Deviation','Observed SPD'),lwd=c(5,5,5,2),col=c('lightgrey','red','blue','black'),bty='n', cex=c(1, 1, 1, 1))
dev.off()

tiff(file="/Users/jch/Desktop/dexp_fit.tiff", units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
