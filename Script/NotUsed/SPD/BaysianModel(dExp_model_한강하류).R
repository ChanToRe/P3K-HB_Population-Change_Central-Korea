...
data <- subset(data,subset = Area == "한강하류")
...
# Plot
tiff(file=here("./Graph/Notused/dexp_HPD(한강하류).tiff"), units='in', res=300, width=8, height=3)
par(mfrow=c(1,3))
postHPDplot(mcmc.double_exp.samples$samples$chain1[, 'r1'], xlab='', ylab='', show.hpd.val=FALSE)
title(main="한강하류(dExp-model):r1", cex=1.2)
...
title(main="한강하류(dExp-model):r2", cex=1.2)
...
title(main="한강하류(dExp-model):c", cex=1.2)
...
tiff(file=here("./Graph/Notused/dexp_model(한강하류).tiff"), units='in', res=300, width=7, height=5)
par(mfrow=c(1, 1))
...
tiff(file=here("./Graph/Notused/dexp_fit(한강하류).tiff"), units='in', res=300, width=7, height=7)
par(mfrow=c(1, 1))
postHPDplot(postPredCor(pp.check.double_exp.cal), xlab="한강하류 PCC",ylab='Density',xlim=c(0,1))
dev.off()
